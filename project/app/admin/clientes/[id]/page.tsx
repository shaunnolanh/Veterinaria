...
export default function ClienteDetallePage() {
  ...
  async function cargarCliente(id: string) {
    try {
      ...
      if (!res.ok) {
        console.error("Error al cargar cliente:", res.status, res.statusText);
        setCliente(null);
        setMascotas([]);
        return;
      }
      ...
      if (!mascotaActiva && data.mascotas?.[0]) {
        setMascotaActiva(data.mascotas[0]);
      }
    } catch (error) {
      console.error("Fallo cargando cliente:", error);
      setCliente(null);
      setMascotas([]);
    }
  }
  ...
  async function cargarConsultas(mascotaId: string) {
    try {
      const res = await fetch(`/api/admin/mascotas/${mascotaId}/historial`, {
        cache: "no-store",
      });

      if (!res.ok) {
        console.error("Error al cargar consultas:", res.status, res.statusText);
        setConsultas([]);
        return;
      }

      const data = await res.json();
      console.log("Respuesta consultas:", data);
      setConsultas(data.consultas || []);
    } catch (error) {
      console.error("Fallo cargando consultas:", error);
      setConsultas([]);
    }
  }
  ...
  async function agregarConsulta(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(
      `/api/admin/mascotas/${mascotaActiva.id}/historial`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const edadMascota = useMemo(() => {
    ...
    const years = Math.floor(
      (Date.now() - new Date(mascotaActiva.fecha_nacimiento).getTime()) /
      ...
      <div className="p-6 max-w-7xl">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
          ...
          <p className="text-gray-500 text-sm mt-1">
            {cliente
              ? `${cliente.nombre} ${cliente.apellido} · ${cliente.telefono} · ${cliente.email || "Sin email"}`
              : "Cargando..."}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              disabled={mascotas.length >= 10}
              onClick={() => setShowMascotaModal(true)}
              className="bg-purpura text-white rounded-xl px-3 py-2 text-xs font-semibold disabled:opacity-40"
            >
              Agregar mascota
            </button>
          </div>
          <div className="flex flex-wrap gap-2">
            {mascotas.map((m) => (
              <button
                ...
                className={`rounded-xl px-3 py-2 text-sm border ${mascotaActiva?.id === m.id ? "bg-purpura/10 border-purpura text-purpura" : "border-gray-200 text-gray-700"}`}
              >
                {m.nombre} · {m.especie}
              </button>
            ))}
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {mascotaActiva.especie} · {mascotaActiva.raza || "Sin raza"} ·{" "}
                  {edadMascota} · {mascotaActiva.sexo || "-"}
                </p>
              </div>
              <button
                onClick={() => setShowConsultaModal(true)}
                className="bg-verde-lima text-gray-900 rounded-xl px-3 py-2 text-xs font-semibold"
              >
                Nueva consulta
              </button>
            </div>
              ...
              {consultas.map((consulta) => (
                ...
                >
                  ...
                  <div className="grid md:grid-cols-2 gap-4 mt-3 text-sm">
                    <div>
                      <p className="font-semibold">Anamnesis</p>
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        Peso: {consulta.peso_kg || "-"}kg · Temp:{" "}
                        {consulta.temperatura_c || "-"}°C · FC:{" "}
                        {consulta.frecuencia_cardiaca || "-"} · FR:{" "}
                    ...
                    <div>
                      <p className="font-semibold">Análisis</p>
                      <p className="text-gray-600">
                        {consulta.analisis_solicitados || "-"}
                      </p>
                    </div>
                    <div>
                      <p className="font-semibold">Seguimiento</p>
                      <p className="text-gray-600">
                        Próxima consulta: {consulta.proxima_consulta || "-"}
                      </p>
                    </div>
                  </div>
                </details>
        ...
        {showMascotaModal && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-30">
            <div className="bg-white rounded-2xl w-full max-w-lg p-5 border border-gray-100">
              <h3 className="font-bold mb-3 text-gray-900">Agregar mascota</h3>
              <form onSubmit={agregarMascota} className="space-y-3">
                <input
                  required
                  placeholder="Nombre"
                  className="w-full border border-gray-200 text-gray-900 rounded-xl px-3 py-2 text-sm"
                  value={formMascota.nombre}
                  onChange={(e) =>
                    setFormMascota((p) => ({ ...p, nombre: e.target.value }))
                  }
                />
                <select
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-gray-900"
                  value={formMascota.especie}
                  onChange={(e) =>
                    setFormMascota((p) => ({ ...p, especie: e.target.value }))
                  }
                >
                ...
                <input
                  placeholder="Raza"
                  className="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400"
                  value={formMascota.raza}
                  onChange={(e) =>
                    setFormMascota((p) => ({ ...p, raza: e.target.value }))
                  }
                />
                <input
                  type="date"
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="border rounded-xl px-4 py-2 text-sm text-gray-900"
                  >
                    Cancelar
                  </button>
                  <button className="bg-purpura text-white rounded-xl px-4 py-2 text-sm">
                    Guardar
                  </button>
                </div>
              </form>
        ...
        {showConsultaModal && mascotaActiva && (
          <div className="fixed inset-0 bg-black/40 flex items-start justify-center p-4 z-30 overflow-y-auto">
            <div className="bg-white rounded-2xl w-full max-w-4xl p-5 border border-gray-100 my-8 max-h-[calc(100vh-2rem)] overflow-y-auto">
              ...
              <form onSubmit={agregarConsulta} className="space-y-4 text-sm">
                <div className="grid md:grid-cols-3 gap-3">
                  <input
                    type="date"
                    className="border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.fecha}
                    onChange={(e) =>
                      setFormConsulta((p) => ({ ...p, fecha: e.target.value }))
                    }
                  />
                  <select
                    className="border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.veterinario_id}
                    onChange={(e) => {
                      const selected = vets.find(
                        (v) => v.id === e.target.value,
                      );
                      setFormConsulta((p) => ({
                        ...p,
                        veterinario_id: e.target.value,
                        veterinario_nombre: selected
                  ...
                  <input
                    placeholder="Veterinario texto libre"
                    className="border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900 placeholder:text-gray-400"
                    value={formConsulta.veterinario_nombre}
                    onChange={(e) =>
                      setFormConsulta((p) => ({
                        ...p,
                        veterinario_nombre: e.target.value,
                      }))
                    }
                  />
                </div>
                ...
                <textarea
                  ...
                  className="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900 placeholder:text-gray-400"
                  value={formConsulta.motivo_consulta}
                  onChange={(e) =>
                    setFormConsulta((p) => ({
                      ...p,
                ...
                <div className="grid md:grid-cols-5 gap-3">
                  ...
                  <input
                    placeholder="Temp (°C)"
                    className="border border-gray-200 rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.temperatura_c}
                    onChange={(e) =>
                      setFormConsulta((p) => ({
                        ...p,
                  ...
                  />
                  <input
                    placeholder="Frec. Card."
                    className="border border-gray-200 rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.frecuencia_cardiaca}
                    onChange={(e) =>
                      setFormConsulta((p) => ({
                        ...p,
                  ...
                  />
                  <input
                    placeholder="Frec. Res."
                    className="border border-gray-200 rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.frecuencia_respiratoria}
                    onChange={(e) =>
                      setFormConsulta((p) => ({
                        ...p,
                  ...
                  />
                  <input
                    placeholder="Condición 1-9"
                    className="border border-gray-200 rounded-xl px-3 py-2 text-gray-900"
                    value={formConsulta.condicion_corporal}
                    onChange={(e) =>
                      setFormConsulta((p) => ({
                        ...p,
                ...
                <textarea
                  ...
                  value={formConsulta.diagnostico_presuntivo}
                  onChange={(e) =>
                    setFormConsulta((p) => ({
                      ...p,
                ...
                <textarea
                  ...
                  className="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900 placeholder:text-gray-400"
                  value={formConsulta.diagnostico_definitivo}
                  onChange={(e) =>
                    setFormConsulta((p) => ({
                      ...p,
                      diagnostico_definitivo: e.target.value,
                    }))
                  }
                />
                <textarea
                ...
                <textarea
                  ...
                  className="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-gray-900 placeholder:text-gray-400"
                  value={formConsulta.indicaciones}
                  onChange={(e) =>
                    setFormConsulta((p) => ({
                      ...p,
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="border border-gray-200 rounded-xl px-4 py-2 text-gray-900 bg-white"
                  >
                    Cancelar
                  </button>
                  <button className="bg-purpura text-white rounded-xl px-4 py-2">
                    Guardar consulta
                  </button>
                </div>
              </form>
