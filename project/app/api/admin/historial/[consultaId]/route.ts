...
export async function PATCH(request: NextRequest, context: { params: Promise<{ consultaId: string }> }) {
  try {
    ...
    const id = sanitizeText(consultaId, 64);
    ...
    const { data, error } = await supabase.from("historial_clinico").update(payload).eq("id", id).select().single();
...
export async function DELETE(_request: NextRequest, context: { params: Promise<{ consultaId: string }> }) {
  try {
    const { consultaId } = await context.params;
    const id = sanitizeText(consultaId, 64);
    ...
    if (error) {
      return NextResponse.json({ error: "No se pudo eliminar la consulta." }, { status: 500 });
    }
